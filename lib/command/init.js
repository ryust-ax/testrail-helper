...
const { resolveDirectory, isFile, beautify } = require("../../utils");
...
module.exports = function (initPath) {
  const testsPath = resolveDirectory(initPath);
  ...
  inquirer
    .prompt([
      ...
      },
      {
        ...
        message:
          "Domain for testrail, not the full api path, no trailing slash.",
      },
      {
        name: "user",
      ...
      const finish = () => {
        ...
        if (config.output) {
          if (!isFile(config.output)) {
            ...
            logger.info(
              `Directory for temporary output files created at "${config.output}"`
            );
          } else {
            logger.info(
              `Directory for temporary output files is already created at "${config.output}"`
            );
          }
        }
      };
